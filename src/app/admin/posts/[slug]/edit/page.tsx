...
export default async function EditPostPage({ params, searchParams }: Props) {
  ...
  // Wiki out-links + backlinks (admin includes drafts)
  const outgoing = extractWikiLinks(post.content).map((l) => ({
    slug: l.slug,
    label: l.label,
  }));
  const adminSources = getAllPosts({ includeDrafts: true }).map((p) => {
    let content = "";
    try {
  ...
  const backlinks = collectBacklinks(post.slug, adminSources, {
    includeDrafts: true,
  });
